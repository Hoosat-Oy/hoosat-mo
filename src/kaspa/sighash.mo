...
//     public let SigHashAll_AnyOneCanPay: SigHashType = 0x81; // Pre-computed 0x01 | 0x80
//     public let SigHashNone_AnyOneCanPay: SigHashType = 0x82; // Pre-computed 0x02 | 0x80
//     public let SigHashSingle_AnyOneCanPay: SigHashType = 0x84; // Pre-computed 0x04 | 0x80;

//     // Check if SigHashType is standard
//     public func is_standard_sighash_type(hashType: SigHashType): Bool {
//     // Debug constant values to ensure they are correct
//     // Debug.print("   🔍 SigHashType constants:");
//     // Debug.print("      SigHashAll: 0x" # Nat8.toText(SigHashAll));
//     // Debug.print("      SigHashNone: 0x" # Nat8.toText(SigHashNone));
//     // Debug.print("      SigHashSingle: 0x" # Nat8.toText(SigHashSingle));
//     // Debug.print("      SigHashAll_AnyOneCanPay: 0x" # Nat8.toText(SigHashAll_AnyOneCanPay));
//     // Debug.print("      SigHashNone_AnyOneCanPay: 0x" # Nat8.toText(SigHashNone_AnyOneCanPay));
//     // Debug.print("      SigHashSingle_AnyOneCanPay: 0x" # Nat8.toText(SigHashSingle_AnyOneCanPay));
//     // Debug.print("   🔍 is_standard_sighash_type input: 0x" # Nat8.toText(hashType));

//     // Explicit comparisons to avoid switch statement issues
//     let result = hashType == SigHashAll or
...
//     // Debug.print("   🔍 Result: " # (if result "true" else "false"));
//     result
//   };
...
//       hashWriter.append(Buffer.fromArray([script_len]));
...
//       Debug.print("❌ Invalid SigHashType: " # Nat8.toText(hashType));
//       return null;
//     };

//     Debug.print("🔢 SCHNORR SIGHASH CALCULATION for input " # Nat.toText(input_index));
...
//     hashWriter.append(Buffer.fromArray(version_bytes));
//     Debug.print("   1. Version: " # Nat.toText(Nat16.toNat(tx.version)) # " -> " # hex_from_array(version_bytes));
...
//     hashWriter.append(Buffer.fromArray(txid_bytes));
//     hashWriter.append(Buffer.fromArray(index_bytes));
//     Debug.print("   5. Input TxID: " # tx.inputs[input_index].previousOutpoint.transactionId);
//     Debug.print("   6. Input Index: " # Nat.toText(Nat32.toNat(tx.inputs[input_index].previousOutpoint.index)) # " -> " # hex_from_array(index_bytes));

//     // Script details
//     let script_version_bytes = nat16_to_bytes(utxo.scriptVersion);
//     hashWriter.append(Buffer.fromArray(script_version_bytes));
//     Debug.print("   7. Script Version: " # Nat.toText(Nat16.toNat(utxo.scriptVersion)) # " -> " # hex_from_array(script_version_bytes));

//     let script_bytes = hex_to_bytes(utxo.scriptPublicKey);
//     let script_len = Nat8.fromNat(script_bytes.size());
//     hashWriter.append(Buffer.fromArray([script_len]));
//     hashWriter.append(Buffer.fromArray(script_bytes));
//     Debug.print("   8. ScriptPubKey Length: " # Nat.toText(script_bytes.size()) # " -> " # hex_from_array([script_len]));
...
//     hashWriter.append(Buffer.fromArray(amount_bytes));
//     Debug.print("   9. Amount: " # Nat64.toText(utxo.amount) # " -> " # hex_from_array(amount_bytes));

//     // Sequence
//     let sequence_bytes = nat64_to_le_bytes(tx.inputs[input_index].sequence);
//     hashWriter.append(Buffer.fromArray(sequence_bytes));
//     Debug.print("  10. Sequence: " # Nat64.toText(tx.inputs[input_index].sequence) # " -> " # hex_from_array(sequence_bytes));

//     // SigOpCount
//     let sigop_count = [tx.inputs[input_index].sigOpCount];
//     hashWriter.append(Buffer.fromArray(sigop_count));
//     Debug.print("  11. SigOpCount: " # Nat.toText(Nat8.toNat(tx.inputs[input_index].sigOpCount)) # " -> " # hex_from_array(sigop_count));

//     // OutputsHash
//     let outputs_hash = get_outputs_hash(tx, input_index, hashType, reusedValues);
//     hashWriter.append(Buffer.fromArray(outputs_hash));
//     Debug.print("  12. OutputsHash: " # hex_from_array(outputs_hash));

//     // LockTime
//     let locktime_bytes = nat64_to_le_bytes(tx.lockTime);
//     hashWriter.append(Buffer.fromArray(locktime_bytes));
//     Debug.print("  13. LockTime: " # Nat64.toText(tx.lockTime) # " -> " # hex_from_array(locktime_bytes));

//     // SubnetworkID
//     let subnetwork_bytes = hex_to_bytes(tx.subnetworkId);
//     hashWriter.append(Buffer.fromArray(subnetwork_bytes));
//     Debug.print("  14. SubnetworkID: " # tx.subnetworkId # " -> " # hex_from_array(subnetwork_bytes));

//     // Gas
//     let gas_bytes = nat64_to_le_bytes(tx.gas);
//     hashWriter.append(Buffer.fromArray(gas_bytes));
//     Debug.print("  15. Gas: " # Nat64.toText(tx.gas) # " -> " # hex_from_array(gas_bytes));

//     // PayloadHash
//     let payload_hash = get_payload_hash(tx, reusedValues);
//     hashWriter.append(Buffer.fromArray(payload_hash));
//     Debug.print("  16. PayloadHash: " # hex_from_array(payload_hash));

//     // SighashType
//     let sighash_type_bytes = [hashType];
//     hashWriter.append(Buffer.fromArray(sighash_type_bytes));
//     Debug.print("  17. SighashType: " # hex_from_array(sighash_type_bytes));

//     let preimage_bytes = Buffer.toArray(hashWriter);
//     Debug.print(" 🔢 Schnorr preimage (" # Nat.toText(preimage_bytes.size()) # " bytes): " # hex_from_array(preimage_bytes));

//     let schnorr_hash = blake2b_256(preimage_bytes, ?transaction_signing_schnorr_domain());
//     Debug.print(" 🎯 Schnorr sighash: " # hex_from_array(schnorr_hash));
//     Debug.print(" 📏 Schnorr sighash length: " # Nat.toText(schnorr_hash.size()) # " bytes");
...
//     reusedValues: SighashReusedValues
//   ): ?[Nat8] {
//     Debug.print("🔢 ECDSA SIGHASH CALCULATION for input " # Nat.toText(input_index));

//     switch (calculate_sighash_schnorr(tx, input_index, utxo, hashType, reusedValues)) {
//       case (null) {
//         Debug.print("❌ Failed to calculate Schnorr sighash");
//         return null;
//       };
//       case (?schnorr_hash) {
//         let domain_hash = transaction_signing_ecdsa_domain_hash();
//         Debug.print("   0. ECDSA Domain Hash: " # hex_from_array(domain_hash));
...
//         Debug.print("   1. ECDSA preimage (" # Nat.toText(final_preimage.size()) # " bytes): " # hex_from_array(final_preimage));

//         let final_sighash = Blob.toArray(Sha256.fromBlob(#sha256, Blob.fromArray(final_preimage)));
//         Debug.print(" 🎯 ECDSA sighash: " # hex_from_array(final_sighash));
//         Debug.print(" 📏 ECDSA sighash length: " # Nat.toText(final_sighash.size()) # " bytes");
